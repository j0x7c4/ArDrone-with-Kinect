...
char image_send_data[320*240*3];
HANDLE m_hSocketImageReadyEvent;
...
{  
  ...
  //创建服务器套接字  
  cmd_receiver = socket(AF_INET,SOCK_DGRAM,0);
  img_sender = socket(AF_INET,SOCK_DGRAM,0);
  //创建地址   
  cmd_receiver_addr.sin_family = AF_INET;  
  cmd_receiver_addr.sin_port = htons(2001);
  cmd_receiver_addr.sin_addr.S_un.S_addr = htonl(INADDR_ANY);

  img_sender_addr.sin_family = AF_INET;  
  img_sender_addr.sin_port = htons(6001);  
  img_sender_addr.sin_addr.S_un.S_addr = inet_addr("127.0.0.1");

  len = sizeof(struct sockaddr);
  bind(cmd_receiver,(struct sockaddr*)&cmd_receiver_addr,len);

  return 0;
}

void SocketUnInit ( ){
  closesocket(cmd_receiver);  
  WSACleanup();  
}


static DWORD WINAPI Socket_ImageSender ( LPVOID pParm ) {
  HRESULT res;
  printf("Image sender starts...\n");
  res = _Socket_ImageSender( );
  printf("Image sender ends...\n");
  return res;
}
int frame_id;
int _Socket_ImageSender ( ){
  char send_buf[10];
  frame_id = 0;
  while ( 1 ) {
    HRESULT res = WaitForSingleObject(m_hSocketImageReadyEvent,INFINITE);
    if ( res == WAIT_FAILED ) break;
    if ( res == WAIT_OBJECT_0 ) {
      sprintf(send_buf,"%d",frame_id);
      //memcpy(send_buf,image_send_data,sizeof(image_send_data));
      sendto(img_sender,send_buf,strlen(send_buf)+1,0,(struct sockaddr*)&img_sender_addr,sizeof(img_sender_addr));
    }
    ResetEvent(m_hSocketImageReadyEvent);
  }
  return 0;
}
...
extern int cmdflying[4];
int _Socket_CommandReceiver ( ) {
  static char cmd[20];
  while(1)  
  {
      //接收数据
	 
    recvfrom(cmd_receiver,recvBuf,128,0,(struct sockaddr*)&addrClient,&len);  
      //ipClient = inet_ntoa(addrClient.sin_addr);  

	  sscanf(recvBuf,"%s %d %d %d %d",cmd, &cmdflying[0], &cmdflying[1], &cmdflying[2], &cmdflying[3]);
    cmdflying[0]= -cmdflying[0];
	  printf("[Pitch %f] [Roll %f] [Yaw %f] [Gaz %f]%\n",1.0*cmdflying[0],1.0*cmdflying[1],1.0*cmdflying[2],1.0*cmdflying[3]);
...
static DWORD WINAPI Socket_CommandReceiver ( LPVOID pParm ) {
  HRESULT res;
  printf("Command receiver starts...\n");
  res = _Socket_CommandReceiver( );
  printf("Command receiver ends...\n");
  return res;
}
