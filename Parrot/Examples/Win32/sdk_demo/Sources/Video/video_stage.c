...
/*Image sender event*/
extern HANDLE m_hSocketImageReadyEvent;
extern uint8_t * FrameBuffer;
extern IplImage *cv_image;
extern CvVideoWriter *cv_writer;
extern char image_send_data[320*240*3];
/*****************************************************************************/
/*
...
C_RESULT output_rendering_device_stage_transform( void *cfg, vp_api_io_data_t *in, vp_api_io_data_t *out)
{
  ...
  vec = (vlib_stage_decoding_config_t*)cfg;
  
  vp_os_mutex_lock(&video_update_lock);
  
  /* Get a reference to the last decoded picture */
  pixbuf_data      = (uint8_t*)in->buffers[0];
  //show front camera video in OpenCV
  w = vec->controller.width; 
  h = vec->controller.height;
  for ( i = 0 ; i<h ; i++ ) {
    p = (cv_image->imageData + i*w*3);
  ...
  sprintf(title_buf,"D:\\ardrone_tmp\\%d.jpg",frame_id);
  cvSaveImage(title_buf,cv_image,0);
  SetEvent(m_hSocketImageReadyEvent);
  
	cvShowImage("Ar.Drone Front Camera",cv_image);

#ifdef VIDEO_RECORD
  cvWriteFrame(cv_writer,cv_image);
#endif
  cvWaitKey(10);
